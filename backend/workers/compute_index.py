...
async def run(days: int) -> None:
    ...
    try:
        ...
        print(f"{'='*64}")
        print(f"  Raw score      : {raw_score if raw_score is not None else '—'} / 100")
        print(f"  Smoothed score : {smoothed_score if smoothed_score is not None else '—'} / 100  (EMA a={EMA_ALPHA})")
